...
exports.list = (req,res) => { //리스트 모듈
	...
	sql = ` SELECT  count(*) cnt FROM tb_board WHERE bid = ? ${where} `;
	conn.query(sql,[body.bid],(err,data)=>{
		if(err) throw err;
		...
		sql = ` SELECT * FROM tb_board WHERE bid = ? ${where} ORDER BY num DESC LIMIT ?, ? `; 
		conn.query(sql,[body.bid, start, end],(err,list)=> {
			if(err) throw err;
...
exports.add = (req,res) => { //등록 프로세스 모듈
	...
	sql = " INSERT INTO  tb_board (bid, title, contents, id, regdate) values (?, ?, ?, ?,now()) ";
	conn.query(sql,
		[body.bid
		, body.title
		, body.contents
