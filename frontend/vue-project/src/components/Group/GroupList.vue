<template>
  <div>
    <div>
      <b-card-group deck class="d-flex flex-row">
        <b-col cols="3" v-for="(group, idx) in myGroups" :key="idx" :group="group">
          ...
          <b-modal :id="'mygroup-' + idx" size="xl" :title="'' + group.groupName"> </b-modal>
        </b-col>
      </b-card-group>
    </div>
    ...
    <b-card-group deck class="d-flex flex-row">
      <b-col cols="3" v-for="(group, idx) in groups" :key="idx" :group="group">
        <!-- @click="goDetail(group)" -->
        <!-- v-b-modal.group-13 -->
        <b-card
          @click="$bvModal.show(`group-${idx}`), getPermission(group.gId)"
        ...
        <b-modal :id="'group-' + idx" size="xl" :title="'' + group.groupName">
          <div>
            <b-jumbotron bg-variant="info" text-variant="white" border-variant="dark">
              <template #header>{{ group.groupName }}</template>
              <b-button id="none" @click="applyGroup(group.gId)" pill variant="primary"
                >그룹 참가하기</b-button
              >
              ...
              <b-button id="ban" pill variant="danger"
                >당신은 이 스터디에서 추방되었습니다</b-button
              >
              <b-button v-if="!loginStatus.token" pill variant="secondary"
                >스터디에 참여하려면 로그인 해주세요</b-button
              >
  ...
  methods: {
    searchGroup: function() {
      ...
      getPermission: function(gId) {
        ...
        axios
          .get(`${SERVER_URL}/group/member/apply/state`, {
            params: { gId: gId, nickname: this.loginStatus.nickname },
          })
          .then((res) => {
      ...
      applyGroup: function(gId) {
        axios
          .post(`${SERVER_URL}/group/member/apply`, {
            gId: gId,
      ...
      deleteGroup: function(gId) {
        axios
          .delete(`${SERVER_URL}/group/delete/`, { headers: { gId: gId } })
          .then((res) => {
            console.log(res);
